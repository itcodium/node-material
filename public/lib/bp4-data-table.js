 ...
 * idea original --> https://github.com/daniel-nagy/md-data-table
 * @license MIT
 * v0.9.12
 */
(function (window, angular, undefined) {
  ...
  angular.module('md-table-pagination.html', []).run(['$templateCache', function($templateCache) {
    ...
    //todo caob ver si hace falta if en el primero (adcInfo)
    
    $templateCache.put('md-table-pagination.html',

        '<span class="label">{{ $pagination.adcInfo }}</span>\n' +
        '\n' +
        ...
        '<span class="label" ng-if="$pagination.showRowsSelect() && $pagination.showPageLineSelect()">{{ $pagination.$label[\'rowsPerPage\'] }}</span>\n' +
        '\n' +
        '<md-select class="md-table-select" ng-if="$pagination.showRowsSelect() && $pagination.showPageLineSelect()" ng-model="$pagination.limit" md-container-class="md-pagination-select" aria-label="Rows" placeholder="{{$pagination.options ? $pagination.options[0] : 5}}">\n' +
  ...
  angular.module('bp4.data.table', ['md.table.templates']);

  angular.module('bp4.data.table').directive('mdBody', mdBody);
  ...
  angular.module('bp4.data.table').directive('mdCell', mdCell);
  ...
  angular.module('bp4.data.table').directive('mdColumn', mdColumn);
  ...
  function mdColumn($compile) {
    ...
    function postLink(scope, element, attrs, ctrls) {
      ...
      function attachSortIcon() {
        ...
        var alignRight = (scope.$parent.columns && scope.$parent.columns.align) ? (scope.$parent.columns.align.toUpperCase() == 'RIGHT') : false;
        if(element.hasClass('md-numeric') || alignRight) {
      ...
      return {
        controller: ['$element', '$q', 'save', '$scope', function ($element, $q, save, $scope) {
          ...
          this.getInput = function () {
            console.log("$scope.editDialog.input -> ",$scope.editDialog.input)
        ...
        (options.controlTemplate ?
                options.controlTemplate :
                ('<input name="input" ng-model="model" md-autofocus type="' + (options.numeric ? 'number' : 'text') + '" placeholder="{{placeholder}} "' + getAttrs() + '>')
        ) +
        ...
        '<a href="javascript:void(0)" class="md-button md-ink-ripple" ng-click="dismiss()">Cancelar</a>' +
        '<md-button type="button" class="md-raised md-primary md-button md-ink-ripple" ng-click="submit()">' +
        '<span>Aceptar</span>' +
        '</md-button>' +
        '</md-dialog-actions>' +
        '</md-edit-dialog>'
  ...
  angular.module('bp4.data.table').directive('mdFoot', mdFoot);
  ...
  angular.module('bp4.data.table').directive('mdHead', mdHead);
    ...
    function postLink(scope, element, attrs, tableCtrl) {
      ...
      function createCheckBox() {
        ...
        if (enableSingleRowSelection()) {
          return angular.element('<th class="md-column md-checkbox-column">')
        } else {
      ...
      function enableSingleRowSelection() {
        return tableCtrl.enableSingleSelect();
      }
  ...
  angular.module('bp4.data.table').directive('mdRow', mdRow);
  ...
  angular.module('bp4.data.table').directive('mdSelect', mdSelect);
  ...
  function mdSelect($compile) {
    ...
    function postLink(scope, element, attrs, ctrls) {
      ...
      self.select = function () {
        ...
        if (tableCtrl.enableSingleSelect()) {
      ...
      scope.$watch(tableCtrl.enableSingleSelect, function (multiple) {
  ...
  angular.module('bp4.data.table').directive('mdTable', mdTable);
  ...
  function mdTable() {
    ...
    function Controller($attrs, $element, $q, $scope) {
      ...
      self.enableSingleSelect = function () {
        return $attrs.singleSelection === '' || $scope.$eval($attrs.singleSelection);
      };
  ...
  angular.module('bp4.data.table').directive('mdTablePagination', mdTablePagination);
  ...
  function mdTablePagination() {
    ...
    function Controller($attrs, $scope) {
      ...
      self.onPaginationChange = function () {
        if(angular.isFunction(self.onPaginate)) {
          self.onPaginate(self.page, self.limit);   /*todo caob*/
        }
      };
      ...
      self.showPageSelect = function () {
        //add caob 20170718
        if (self.total > 500) return false;
        if($attrs.hasOwnProperty('mdPageSelect') && $attrs.mdPageSelect === '') {
          return true;
        }
        return self.pageSelect;
      };
      self.showPageLineSelect = function () {
        var res=true;
          if(self.hideLineasPagina==true){
            res=false;
          }
        return res;
      };
      ...
      self.showRowsSelect = function () {
        if($attrs.hasOwnProperty('mdRowsSelect') && $attrs.mdRowsSelect === '') {
          return true;
        }
    ...
    return {
      bindToController: {
        ...
        label: '@?mdLabel',
        limit: '=mdLimit',
        hideLineasPagina: '=mdHideLineasPagina',
        page: '=mdPage',
        pageSelect: '=?mdPageSelect',
        rowsSelect: '=?mdRowsSelect',
  ...
  angular.module('bp4.data.table').directive('mdTableProgress', mdTableProgress);
