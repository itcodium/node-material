...
(function () {
  ...
  angular.module('bp4-tree', [])
    .constant('treeConfig', {
      treeClass: 'bp4-tree',
      emptyTreeClass: 'bp4-tree-empty',
      hiddenClass: 'bp4-tree-hidden',
      nodesClass: 'bp4-tree-nodes',
      nodeClass: 'bp4-tree-node',
      handleClass: 'bp4-tree-handle',
      placeholderClass: 'bp4-tree-placeholder',
      dragClass: 'bp4-tree-drag',
      dragThreshold: 3,
      levelThreshold: 30,
  ...
  angular.module('bp4-tree')
    .controller('TreeNodeController', ['$scope', '$element',
      function ($scope, $element) {
        ...
        $scope.$parentNodeScope = null; // bp4TreeNode Scope of parent node;
        $scope.$childNodesScope = null; // bp4TreeNodes Scope of child nodes.
        $scope.$parentNodesScope = null; // bp4TreeNodes Scope of parent nodes.
        $scope.$treeScope = null; // bp4Tree scope
        $scope.$handleScope = null; // it's handle scope
        $scope.$type = 'bp4TreeNode';
...
(function () {
  ...
  angular.module('bp4-tree')
    .directive('bp4Tree', ['treeConfig', '$window',
      function (treeConfig, $window) {
        return {
          ...
          link: function (scope, element, attrs, ctrl) {
             ...
             * By default, bp4-tree will check that 'data-nodrop-enabled' is not set for the
             * destination bp4-tree-nodes, and that the 'max-depth' attribute will not be exceeded
             * if it is set on the bp4-tree or bp4-tree-nodes.
             ...
             * @param sourceNodeScope Scope of the bp4-tree-node being dragged
             * @param destNodesScope Scope of the bp4-tree-nodes where the node is hovering
            ...
            scope.$watch(attrs.bp4Tree, function (newVal, oldVal) {
...
(function () {
  ...
  angular.module('bp4-tree')
    .directive('bp4TreeHandle', ['treeConfig',
      function (treeConfig) {
        return {
          require: '^bp4TreeNode',
          restrict: 'A',
...
(function () {
  ...
  angular.module('bp4-tree')
    ...
    .directive('bp4TreeNode', ['treeConfig', 'bp4TreeHelper', '$window', '$document', '$timeout', '$q', '$rootElement',
      function (treeConfig, bp4TreeHelper, $window, $document, $timeout, $q, $rootElement) {
        return {
          require: ['^bp4TreeNodes', '^bp4Tree'],
          restrict: 'A',
          ...
          link: function (scope, element, attrs, controllersArr) {
            ...
            scope.collapsed = !!bp4TreeHelper.getNodeAttribute(scope, 'collapsed') || treeConfig.defaultCollapsed;
            ...
            scope.$watch('collapsed', function (val) {
              bp4TreeHelper.setNodeAttribute(scope, 'collapsed', val);
              attrs.$set('collapsed', val);
            });

            scope.$on('bp4-tree:collapse-all', function () {
              scope.collapsed = true;
            });

            scope.$on('bp4-tree:expand-all', function () {
              scope.collapsed = false;
            });
            ...
            dragStart = function (e) {
              ...
              if (e.bp4TreeDragging || (e.originalEvent && e.originalEvent.bp4TreeDragging)) {
                return;
              }
              ...
              // if the target element is a child element of a bp4-tree-handle,
              // use the containing handle element as target element
              isHandleChild = bp4TreeHelper.treeNodeHandlerContainerOfElement(eventElm);
              if (isHandleChild) {
                eventElm = angular.element(isHandleChild);
              }

              cloneElm = element.clone();
              isTreeNode = bp4TreeHelper.elementIsTreeNode(eventElm);
              isTreeNodeHandle = bp4TreeHelper.elementIsTreeNodeHandle(eventElm);

              if (!isTreeNode && !isTreeNodeHandle) {
                return;
              }

              if (isTreeNode && bp4TreeHelper.elementContainsTreeNodeHandler(eventElm)) {
                return;
              }
              ...
              while (el && el[0] && el[0] !== element) {
                if (bp4TreeHelper.nodrag(el)) { // if the node mark as `nodrag`, DONOT drag it.
                  return;
                }
                el = el.parent();
              }
              ...
              e.bp4TreeDragging = true; // stop event bubbling
              if (e.originalEvent) {
                e.originalEvent.bp4TreeDragging = true;
              }
              e.preventDefault();
              eventObj = bp4TreeHelper.eventObj(e);

              firstMoving = true;
              dragInfo = bp4TreeHelper.dragInfo(scope);
              ...
              pos = bp4TreeHelper.positionStarted(eventObj, element);
              placeElm.css('height', bp4TreeHelper.height(element) + 'px');

              dragElm = angular.element($window.document.createElement(scope.$parentNodesScope.$element.prop('tagName')))
                .addClass(scope.$parentNodesScope.$element.attr('class')).addClass(config.dragClass);
              dragElm.css('width', bp4TreeHelper.width(element) + 'px');
              dragElm.css('z-index', 9999);

              // Prevents cursor to change rapidly in Opera 12.16 and IE when dragging an element
              hStyle = (element[0].querySelector('.bp4-tree-handle') || element[0]).currentStyle;
              if (hStyle) {
                document.body.setAttribute('bp4-tree-cursor', $document.find('body').css('cursor') || '');
            ...
            dragMove = function (e) {
              var eventObj = bp4TreeHelper.eventObj(e),
                prev,
                next,
                leftElmPos,
                topElmPos,
              ...
              if (dragElm) {
                ...
                bp4TreeHelper.positionMoved(e, pos, firstMoving);
                if (firstMoving) {
                  firstMoving = false;
                  return;
                }
                ...
                decrease = (bp4TreeHelper.offset(dragElm).left - bp4TreeHelper.offset(placeElm).left) >= config.threshold;
                ...
                // if the target element is a child element of a bp4-tree-handle,
                // use the containing handle element as target element
                isHandleChild = bp4TreeHelper.treeNodeHandlerContainerOfElement(targetElm);
                if (isHandleChild) {
                ...
                outOfBounds = !bp4TreeHelper.elementIsTreeNodeHandle(targetElm) &&
                              !bp4TreeHelper.elementIsTreeNode(targetElm) &&
                              !bp4TreeHelper.elementIsTreeNodes(targetElm) &&
                              !bp4TreeHelper.elementIsTree(targetElm) &&
                              !bp4TreeHelper.elementIsPlaceholder(targetElm);
                ...
                // move vertical
                if (!pos.dirAx) {
                  if (bp4TreeHelper.elementIsTree(targetElm)) {
                    targetNode = targetElm.controller('bp4Tree').scope;
                  } else if (bp4TreeHelper.elementIsTreeNodeHandle(targetElm)) {
                    targetNode = targetElm.controller('bp4TreeHandle').scope;
                  } else if (bp4TreeHelper.elementIsTreeNode(targetElm)) {
                    targetNode = targetElm.controller('bp4TreeNode').scope;
                  } else if (bp4TreeHelper.elementIsTreeNodes(targetElm)) {
                    targetNode = targetElm.controller('bp4TreeNodes').scope;
                  } else if (bp4TreeHelper.elementIsPlaceholder(targetElm)) {
                    targetNode = targetElm.controller('bp4TreeNodes').scope;
                  } else if (targetElm.controller('bp4TreeNode')) {
                  ...
                  if (targetNode.$type == 'bp4Tree' && targetNode.dragEnabled) {
                    isEmpty = targetNode.isEmpty(); // Check if it's empty tree
                  }

                  if (targetNode.$type == 'bp4TreeHandle') {
                    targetNode = targetNode.$nodeScope;
                  }

                  if (targetNode.$type != 'bp4TreeNode'
                    && !isEmpty) { // Check if it is a bp4TreeNode or it's an empty tree
                    return;
                  }
                  ...
                  if (isEmpty) { // it's an empty tree
                    ...
                    targetElm = targetNode.$element; // Get the element of bp4-tree-node
                    targetOffset = bp4TreeHelper.offset(targetElm);
                    targetBefore = targetNode.horizontal ? eventObj.pageX < (targetOffset.left + bp4TreeHelper.width(targetElm) / 2)
                      : eventObj.pageY < (targetOffset.top + bp4TreeHelper.height(targetElm) / 2);
              ...
              scope.$treeScope.$apply(function () {
                $q.when(scope.$treeScope.$callbacks.beforeDrop(dragEventArgs))
                    ...
                    .finally(function () {
                      ...
                      var oldCur = document.body.getAttribute('bp4-tree-cursor');
                      if (oldCur !== null) {
                        $document.find('body').css({'cursor': oldCur});
                        document.body.removeAttribute('bp4-tree-cursor');
                      }
                    });
              });
            };
...
(function () {
  ...
  angular.module('bp4-tree')
    .directive('bp4TreeNodes', ['treeConfig', '$window',
      function (treeConfig) {
        return {
          require: ['ngModel', '?^bp4TreeNode', '^bp4Tree'],
          restrict: 'A',
          scope: true,
          ...
          link: function (scope, element, attrs, controllersArr) {
            ...
            if (treeNodeCtrl) {
              ...
              // find the root nodes if there is no parent node and have a parent bp4-tree
...
(function () {
  ...
  /**
   * @ngdoc service
   * @name bp4-tree.service:bp4TreeHelper
   ...
   * bp4-tree.
   */
    .factory('bp4TreeHelper', ['$document', '$window', 'treeConfig',
      function ($document, $window, treeConfig) {
        return {
          ...
          /**
           * @ngdoc method
           * @methodOf bp4-tree.service:$nodrag
          ...
          /**
           * @ngdoc method
           * @name bp4-tree#height
           * @methodOf bp4-tree.service:bp4TreeHelper
           *
           * @description
          ...
          /**
           * @ngdoc method
           * @name bp4-tree#offset
           * @methodOf ui.nestedSortable.service:bp4TreeHelper
           *
           * @description
          ...
          elementIsTreeNode: function (element) {
            return typeof element.attr('bp4-tree-node') !== 'undefined';
          },

          elementIsTreeNodeHandle: function (element) {
            return typeof element.attr('bp4-tree-handle') !== 'undefined';
          },
          elementIsTree: function (element) {
            return typeof element.attr('bp4-tree') !== 'undefined';
          },
          elementIsTreeNodes: function (element) {
            return typeof element.attr('bp4-tree-nodes') !== 'undefined';
          },
          ...
          elementContainsTreeNodeHandler: function (element) {
            return element[0].querySelectorAll('[bp4-tree-handle]').length >= 1;
          },
          treeNodeHandlerContainerOfElement: function (element) {
            return findFirstParentElementWithAttribute('bp4-tree-handle', element[0]);
          }
        };
      }
    ]);
